				...
				for interfaceHeader, objBook := range objResult.(map[string]interface{}) {
					if interfaceHeader == "bids" {
						for _, objBid := range objBook.([]interface{}) {
							...
							client.HSet("OrderBid:"+strings.Split(strPair, ":")[1]+"#"+epoch, "Price", objBid.([]interface{})[0])
							client.HSet("OrderBid:"+strings.Split(strPair, ":")[1]+"#"+epoch, "Volume", objBid.([]interface{})[1])
						}
						...
						for _, objAsk := range objBook.([]interface{}) {
							...
							client.HSet("OrderAsk:"+strings.Split(strPair, ":")[1]+"#"+epoch, "Price", objAsk.([]interface{})[0])
							client.HSet("OrderAsk:"+strings.Split(strPair, ":")[1]+"#"+epoch, "Volume", objAsk.([]interface{})[1])
						}
					}
				}
			}
